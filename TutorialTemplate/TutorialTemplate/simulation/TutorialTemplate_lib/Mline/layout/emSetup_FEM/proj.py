...
class projImporter():
	...
	def create_materials(self,materialForEachLayer=False):
		...
		extraMaterialProperties=(162,None,False,False) # (priority,thickness,modelTypeForMetals,convertedToResistance)
		material=ADSmaterialMap.get((ADSmaterialName,extraMaterialProperties),None)
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
		...
		ADSmaterialName=["Copper","bottom"][materialForEachLayer]
		extraMaterialProperties=(160,1e-05,False,True) # (priority,thickness,modelTypeForMetals,convertedToResistance)
		material=ADSmaterialMap.get((ADSmaterialName,extraMaterialProperties),None)
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
			material=self.session.create_material(name=EMProMaterialName, color=(117,128,1,255), thickness="1e-05 m", resistance=1.72413793103448e-08/1e-05, permeability=1)
			try:
				material.details.electricProperties.parameters.thickness = "1e-05 m"
			except AttributeError:
				pass
			self._setModelTypeForMetals(material,False)
			try:
			...
			EMProMaterialName=material.name
		self.materials["bottom"]=material
		layerEMProMaterialNameMap["bottom"]=EMProMaterialName
		ADSmaterialName=["fr4custom","__SubstrateLayer1"][materialForEachLayer]
	...
	def initNetlists(self):
		netlistNames = ['net_0','net_1_N__1_N__3']
	...
	def create_geometry(self,topAssembly):
		...
		assembly=empro.geometry.Assembly()
		part=empro.geometry.Model()
		simBox = empro.geometry.Box( _createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox-xLowerBoundingBox", "abs((0-xLowerExtension)-(0.05+xUpperExtension))"), _createIfToggleExtensionToBoundingBoxExpression("zUpperBoundingBox-zLowerBoundingBox", "((((stack_fr4custom_layer_3_Z) + (zUpperExtension)) - ((stack_fr4custom_layer_1_Z) - (zLowerExtension))))"), _createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox-yLowerBoundingBox" , " abs((-0.01-yLowerExtension)-(0.01+yUpperExtension))"))
		part.recipe.append(simBox)
		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(_createIfToggleExtensionToBoundingBoxExpression("(xUpperBoundingBox+xLowerBoundingBox)/2", "(0.05+xUpperExtension+0-xLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("(yUpperBoundingBox+yLowerBoundingBox)/2", "(0.01+yUpperExtension+-0.01-yLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("zLowerBoundingBox","(((stack_fr4custom_layer_1_Z) - (zLowerExtension)) - (0))")))
		...
		self.session.adjust_view()
		assembly=empro.geometry.Assembly()
		pointString='0.05+xUpperExtension#-0.01-yLowerExtension;0.05+xUpperExtension#0.01+yUpperExtension;0-xLowerExtension#0.01+yUpperExtension;0-xLowerExtension#-0.01-yLowerExtension'
		sketch = self._create_sketch(pointString)
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex0",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","0-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.01-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex1",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.05+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.01-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex2",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.05+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox","0.01+yUpperExtension"),0)))
		...
		part=empro.geometry.Model()
		part.recipe.append(empro.geometry.Extrude(sketch,"(stack_fr4custom_layer_3_Z) - (stack_fr4custom_layer_1_Z)",V(0,0,1)))
		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(0,0,"(stack_fr4custom_layer_1_Z) - (0)"))
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['0.05#-0.00075;0.05#0.00075;0#0.00075;0#-0.00075']
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['0.05#-0.01;0.05#0.01;0#0.01;0#-0.01']
		part = self._create_cover(pointStrings)

		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(0,0,"(mask_bottom_Zmin) - (0)"))
		part.setAttribute('LtdLayerNumber', 40)
		part.meshParameters=empro.mesh.ModelMeshParameters()
		part.meshParameters.priority=160
		empro.toolkit.applyMaterial(part,self.materials["bottom"])
		assembly.append(part)
		self.addShortcut(0,part)
		self._setAssemblyMeshSettings(assembly,0,0,0)
		assembly.name="bottom"
		topAssembly.append(assembly)
		return symbPinData
	...
	def getMaskHeights(self,parameterized=False):
		...
		mask_heights[40]=(0, 0)
		mask_heights_parameterized[40]=("(mask_bottom_Zmin) - (0)", "(mask_bottom_Zmax) - (0)")
	...
	def create_ports( self, topAssembly, includeInvalidPorts=True, symbPinData=None):
		...
		tail=V("(0) - (0)","(0) - (0)","(((mask_bottom_Zmax) + (mask_bottom_Zmin)) / (2)) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0) - (0)","(-0.00075) - (0)","(((mask_bottom_Zmax) + (mask_bottom_Zmin)) / (2)) - (0)")
		extent.endPoint2Position=V("(0) - (0)","(0.00075) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		...
		head=V("(0.05) - (0)","(0) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(0.05) - (0)","(0) - (0)","(((mask_bottom_Zmax) + (mask_bottom_Zmin)) / (2)) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0.05) - (0)","(-0.00075) - (0)","(((mask_bottom_Zmax) + (mask_bottom_Zmin)) / (2)) - (0)")
		extent.endPoint2Position=V("(0.05) - (0)","(0.00075) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
	...
	def create_parameters(self):
		self._create_parameter("stack_fr4custom_layer_1_Z", "0 mm", "Z of topology level (level 1 of stack fr4custom)",True,fixGridAxis='Z')
		self._create_parameter("stack_fr4custom_layer_3_Z", "0.8 mm", "Z of topology level (level 3 of stack fr4custom)",True,fixGridAxis='Z')
		...
		self._create_parameter("mask_bottom_Zmin",str("0 mm"),"Zmin of mask bottom",True,fixGridAxis='Z')
		self._create_parameter("mask_bottom_Zmax","mask_bottom_Zmin","Zmax of mask bottom",True)
		self._create_parameter("mask_cond_Zmin",str("0.8 mm"),"Zmin of mask cond",True,fixGridAxis='Z')
